    ...
    render() {
        ...
        return (
            <div className="alert alert-info">
                {cartItems.length === 0
                    ? "Basket is empty" :
                    <div>You have {cartItems.length} diferent items in the basket. <hr /></div>
                }
                {cartItems.length > 0 &&
                    <div>
                        ...
                        <button onClick={(e) => { this.props.handleBuy(e); alert('Info: You have bought items.') }} className="btn btn-primary">pay</button>
                    </div>
                }
            </div>
        )
